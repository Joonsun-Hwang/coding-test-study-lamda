...
def solution(words, queries):
    ...
    for idx, query in enumerate(queries):
        ...
        if start_idx == 0:
            answer[idx] = len([x for x in words if len(x) == len_query and query == x[len(x) - len(query):]])
        else:
            answer[idx] = len([x for x in words if len(x) == len_query and query == x[:len(query)]])
