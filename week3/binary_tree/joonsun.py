...
class BinaryTree(object):
    ...
    def build_tree(self, nodeinfo):
        node_sorted = sorted(
            nodeinfo, key=lambda l: l[1], reverse=True
        )  # y값 기준 내림차순 정렬
        ...
        for c_n in node_sorted:
            ...
            while parent_node is not None:
                cand_node = [
                    n
                    for n in cand_node
                    if n[1] < current_node.y
                    and (current_node.x > parent_node.x and n[0] > parent_node.x)
                    or (current_node.x < parent_node.x and n[0] < parent_node.x)
                ]
            ...
            if left_cand_node:
                left_node_x = sorted(left_cand_node, key=lambda l: l[1], reverse=True)[
                    0
                ][0]
            ...
            if right_cand_node:
                right_node_x = sorted(
                    right_cand_node, key=lambda l: l[1], reverse=True
                )[0][0]
